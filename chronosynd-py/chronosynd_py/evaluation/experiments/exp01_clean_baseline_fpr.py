"""Experiment 01, clean-baseline false-positive rate. Under unpoisoned
windows, what FPR do NaiveBaseline and Sediment produce. This is the
clean-conditions reference point everything else is measured against"""
...
def _results_path() -> Path:
    ...
    return results_dir / "exp01_clean_baseline_fpr.csv"


def _estimator_factories() -> dict[str, EstimatorFactory]:
    return {
        "naive": NaiveBaseline,
        "sediment_trim10": partial(Sediment, trim_fraction=0.1),
        "sediment_trim30": partial(Sediment, trim_fraction=0.3),
        "sediment_trim50": partial(Sediment, trim_fraction=0.5),
    }
...
def main() -> None:
    ...
    print(f"exp01_clean_baseline_fpr: wrote {len(samples)} rows to {path}")
