"""Figure 08, four-way ablation under burst poisoning. Target score and
target-to-benign ratio across naive, consensus, anomaly-within, and
Sediment when the adversary concentrates the budget contiguously"""
...
def _csv_path() -> Path:
    ...
    return repo_root / "evaluation" / "results" / "exp08_burst_alternatives_ablation.csv"
...
def main() -> None:
    ...
    target_fig = _plot(
        target_series,
        title="Target score under burst poisoning",
        ylabel="Target score",
        use_log_y=True,
    )
    save_figure(target_fig, output_dir / "fig08_target_score.pdf")
    plt.close(target_fig)

    ratio_series = aggregate_csv(csv_path, metric=_score_ratio)
    ratio_fig = _plot(
        ratio_series,
        title="Score ratio under burst poisoning",
        ylabel="target / median(benign)",
    )
    save_figure(ratio_fig, output_dir / "fig08_score_ratio.pdf")
    plt.close(ratio_fig)

    detect_series = aggregate_csv(csv_path, metric="detection_rate")
    detect_fig = _plot(
        detect_series,
        title="Detection rate at 5 percent FPR, burst attack",
        ylabel="Detection rate",
    )
    save_figure(detect_fig, output_dir / "fig08_detection_rate.pdf")
    plt.close(detect_fig)

    print(f"fig08_burst_alternatives_ablation: wrote 3 PDFs to {output_dir}")
