"""Figure 07, four-way ablation under poisoning. Target score and
target-to-benign ratio across naive, consensus, anomaly-within, and
Sediment, written as PDFs to `paper/figures/`"""
...
def _csv_path() -> Path:
    ...
    return repo_root / "evaluation" / "results" / "exp07_alternatives_ablation.csv"
...
def main() -> None:
    ...
    target_fig = _plot(
        target_series,
        title="Target score, four-way ablation",
        ylabel="Target score",
        use_log_y=True,
    )
    save_figure(target_fig, output_dir / "fig07_target_score.pdf")
    plt.close(target_fig)

    ratio_series = aggregate_csv(csv_path, metric=_score_ratio)
    ratio_fig = _plot(
        ratio_series,
        title="Score ratio, four-way ablation",
        ylabel="target / median(benign)",
    )
    save_figure(ratio_fig, output_dir / "fig07_score_ratio.pdf")
    plt.close(ratio_fig)

    detect_series = aggregate_csv(csv_path, metric="detection_rate")
    detect_fig = _plot(
        detect_series,
        title="Detection rate at 5 percent FPR",
        ylabel="Detection rate",
    )
    save_figure(detect_fig, output_dir / "fig07_detection_rate.pdf")
    plt.close(detect_fig)

    print(f"fig07_alternatives_ablation: wrote 3 PDFs to {output_dir}")
