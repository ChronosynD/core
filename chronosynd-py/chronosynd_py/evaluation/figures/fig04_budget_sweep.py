"""Figure 04, budget-sweep figures derived from exp04's CSV. Produces
the score-collapse, score-ratio, and detection-rate panels under
`paper/figures/`"""
...
def _csv_path() -> Path:
    ...
    return repo_root / "evaluation" / "results" / "exp04_budget_sweep.csv"
...
def main() -> None:
    ...
    target_fig = _plot(
        target_series,
        title="Target score under poisoning",
        ylabel="Target score",
        use_log_y=True,
    )
    save_figure(target_fig, output_dir / "fig04_target_score.pdf")
    plt.close(target_fig)

    ratio_series = aggregate_csv(csv_path, metric=_score_ratio)
    ratio_fig = _plot(
        ratio_series,
        title="Target-to-benign score ratio",
        ylabel="target / median(benign)",
    )
    save_figure(ratio_fig, output_dir / "fig04_score_ratio.pdf")
    plt.close(ratio_fig)
    ...
    save_figure(detect_fig, output_dir / "fig04_detection_rate.pdf")
    plt.close(detect_fig)

    print(f"fig04_budget_sweep: wrote 3 PDFs to {output_dir}")
